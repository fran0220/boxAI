...
|----|--------|--------|-------|
| P0-1 | Confirm prod OAuth provider **redirect_uri** includes apex (or dual-register console + apex) | **Residual (ops IdP)** | Prod (2026-07-17): only **Google** enabled. Edge allowlist now proxies apex `auth/oauth/*` (P0-3). **Fixed:** `google_oauth_client_id` was corrupted (trailing JSON junk) — sanitized in DB. Backend `google_oauth_redirect_url` still **console** callback. OAuth state cookies are host-bound → start host must match callback host. **Need Google Cloud Console:** add `https://you-box.com/api/v1/auth/oauth/google/callback`, then set `google_oauth_redirect_url` to that apex URI so apex Login can complete. Do not switch redirect_url until IdP lists the apex URI. |
