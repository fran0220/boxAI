...
import { EditIcon, HelpCircleIcon, HistoryIcon, InstallIcon } from './icons'
...
export default function Header() {
  ...
  const { hasUpdate, latestRelease } = useVersionCheck()
  const [showHelp, setShowHelp] = useState(false)
  const [installPrompt, setInstallPrompt] = useState<BeforeInstallPromptEvent | null>(null)
  // BOXAI embed: never show install/PWA affordance inside Creator shell
  ...
  return (
    <>
      {/* BOXAI: sit below CreateLayout (h-12); title rebranded */}
      <header data-no-drag-select className={`safe-area-top fixed left-0 right-0 z-40 top-12 bg-[var(--bx-bg-elevated,#0c1013)]/90 dark:bg-gray-950/80 backdrop-blur border-b border-[var(--bx-border,rgba(255,255,255,0.08))] dark:border-white/[0.08] transition-transform duration-300 ease-in-out ${appMode === 'agent' && !agentMobileHeaderVisible ? '-translate-y-full sm:translate-y-0' : 'translate-y-0'}`}>
          ...
          <div className="flex-1 min-w-0 pr-2 flex items-center gap-2">
            <h1 className="inline-flex min-w-0 items-start relative mr-2">
              ...
              {hasUpdate && latestRelease ? (
                <span className="absolute -right-1 -top-1 translate-x-full -translate-y-1/4 px-1 py-0.5 rounded-[4px] border border-red-500/30 text-[9px] font-black bg-red-500 text-white leading-none shadow-sm">
                  NEW
                </span>
              ) : null}
            </h1>
