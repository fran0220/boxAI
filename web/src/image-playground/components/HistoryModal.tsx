...
function formatTime(value: number) {
  ...
  if (time >= startOfToday) return getPg().today
  if (time >= startOfYesterday) return getPg().yesterday
  if (time >= startOfWeek) return getPg().thisWeek
  return getPg().earlier
}
...
export default function HistoryModal({ onClose, ignoreOutsideClickRef }: HistoryModalProps) {
      ...
      checkbox: relatedTaskIds.length > 0
        ? {
            label: generatedImageCount > 0
              ? t('deleteConvWithImages', { count: generatedImageCount })
              : t('deleteConvWithTasks', { count: relatedTaskIds.length }),
            tone: 'danger',
          }
        : undefined,
  ...
  return (
    <div 
      ref={modalRef}
      className="absolute top-12 left-0 w-80 sm:w-96 max-w-[calc(100vw-2rem)] max-h-[70vh] bg-white dark:bg-[var(--bx-bg-elevated)] rounded-xl shadow-2xl overflow-hidden flex flex-col border border-gray-200 dark:border-white/10 z-50 text-gray-900 dark:text-gray-200 animate-dropdown-down"
    >
      <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-white/10 shrink-0">
        ...
        />
        <HistoryActionButton tooltip={pg.close} onClick={onClose} className="p-1 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg text-gray-500 dark:text-gray-400 transition-colors">
          <CloseIcon className="w-4 h-4" />
        </HistoryActionButton>
      </div>
      <div className="flex-1 overflow-y-auto p-2 space-y-1 overscroll-contain">
        {filteredConversations.length === 0 && (
          <div className="px-3 py-8 text-center text-sm text-gray-500">{pg.noMatchingChats}</div>
        )}
