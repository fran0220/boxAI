...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      appMode: 'gallery',
      setAppMode: (appMode) => {
        // BOXAI: gallery → Images/gpt-image-2; agent → Responses/gpt-5.4 (platform defaults)
        const applyModeProfile = (rawSettings: AppSettings, mode: 'gallery' | 'agent') => {
          const settings = normalizeSettings(rawSettings)
          const apiMode = mode === 'agent' ? 'responses' as const : 'images' as const
          const model = mode === 'agent' ? 'gpt-5.4' : 'gpt-image-2'
        ...
        if (appMode === 'gallery') {
          ...
          set((s) => ({
            appMode,
            settings: applyModeProfile(s.settings, 'gallery'),
            agentInputDrafts,
            galleryInputDraft,
            ...
            ...(s.appMode === 'agent' ? restoreGalleryInputDraftState(galleryInputDraft) : {}),
          }))
          return
        }
        ...
        const galleryInputDraft = saveGalleryInputDraft(state)
        set((s) => ({
          settings: applyModeProfile(s.settings, 'agent'),
          appMode: 'agent',
          galleryInputDraft,
          ...
          selectedTaskIds: [],
          selectedFavoriteCollectionIds: [],
          ...restoreAgentInputDraftState(s.agentInputDrafts, s.activeAgentConversationId),
        }))
      },
...
async function continueRecoveredAgentRound(taskId: string) {
  ...
  try {
    ...
    const _maxToolCalls = Number.isFinite(normalizedSettings.agentMaxToolRounds)
