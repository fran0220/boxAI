...
function createHeaders(profile: ApiProfile, requestUrl: string): Record<string, string> {
  // BOXAI: live session JWT only for gateway origin
  return {
    ...resolveAuthHeaders(profile.apiKey, requestUrl),
...
export async function callAgentResponsesApi(opts: {
  ...
  try {
    ...
    const agentUrl = buildApiUrl(profile.baseUrl, 'responses', proxyConfig, useApiProxy)
    const response = await fetch(agentUrl, {
      method: 'POST',
      headers: createHeaders(profile, agentUrl),
      cache: 'no-store',
...
export async function callAgentConversationTitleApi(opts: {
  ...
  try {
    ...
    const titleUrl = buildApiUrl(profile.baseUrl, 'responses', proxyConfig, useApiProxy)
    const response = await fetch(titleUrl, {
      method: 'POST',
      headers: createHeaders(profile, titleUrl),
      cache: 'no-store',
      body: JSON.stringify({
        model: profile.model || settings.model,
...
export async function callBatchImageSingle(opts: {
  ...
  try {
    ...
    const batchUrl = buildApiUrl(profile.baseUrl, 'responses', proxyConfig, useApiProxy)
    const response = await fetch(batchUrl, {
      method: 'POST',
      headers: createHeaders(profile, batchUrl),
      cache: 'no-store',
      body: JSON.stringify(body),
      signal: controller.signal,
    })
