...
const SHOW_DEFAULT_CONFIG_ONLY = true // BOXAI: lock provider/profile UI
...
const DEFAULT_BASE_URL = DEFAULT_API_URL_PATCH?.baseUrl ?? getBoxaiGatewayBaseUrl()
...
export function isDefaultConfigOnlyEnabled(): boolean {
  // BOXAI: always lock to the embedded gateway profile (no multi-provider UI).
  return SHOW_DEFAULT_CONFIG_ONLY
}

/** Force OpenAI-compatible gateway profile; never persist or forward third-party keys. */
function lockProfileToBoxaiGateway(profile: ApiProfile): ApiProfile {
  return {
    ...profile,
    provider: 'openai',
    baseUrl: getBoxaiGatewayBaseUrl(),
    apiKey: '',
    apiProxy: false,
  }
}
...
export function getAgentTextApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile | null {
  // BOXAI: always platform profile (no independent agent API configs)
...
export function getAgentImageApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile | null {
  return getAgentTextApiProfile(settings)
}
...
export function getActiveApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile {
  ...
  // BOXAI: always route through gateway; model/mode/stream prefs preserved from UI.
  return lockProfileToBoxaiGateway({
    ...profile,
...
export const DEFAULT_SETTINGS: AppSettings = normalizeSettings({
  ...
  reuseTaskApiProfileTemporarily: false, // BOXAI: single platform profile
  ...
  agentApiConfigMode: 'off', // BOXAI: never multi-provider agent API
