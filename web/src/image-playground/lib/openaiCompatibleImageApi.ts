...
function createRequestHeaders(profile: ApiProfile, requestUrl: string): Record<string, string> {
  // BOXAI: inject live session JWT only for gateway origin (never persist apiKey).
  return resolveAuthHeaders(profile.apiKey, requestUrl)
}
...
async function callImagesApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  const generationUrl = buildApiUrl(profile.baseUrl, paths.generationPath, proxyConfig, useApiProxy)
  ...
  try {
    ...
    if (isEdit) {
      ...
      response = await fetch(editUrl, {
        method: 'POST',
        headers: createRequestHeaders(profile, editUrl),
        cache: 'no-store',
        body: formData,
      ...
      response = await fetch(generationUrl, {
        method: 'POST',
        headers: {
          ...createRequestHeaders(profile, generationUrl),
...
async function submitCustomRequest(mapping: CustomProviderSubmitMapping, opts: CallApiOptions, profile: ApiProfile, controller: AbortController, proxyConfig: ReturnType<typeof readClientDevProxyConfig>, useApiProxy: boolean): Promise<unknown> {
  ...
  const requestUrl = buildApiUrl(profile.baseUrl, path, proxyConfig, useApiProxy)
  // BOXAI: custom providers only get JWT if they hit gateway origin
  const headers: Record<string, string> = { ...createRequestHeaders(profile, requestUrl) }
...
async function pollCustomTaskResult(
  ...
  while (true) {
    ...
    const pollUrl = buildApiUrl(profile.baseUrl, taskPath, proxyConfig, false)
    let taskPayload: unknown
    try {
      const taskResponse = await fetch(pollUrl, {
        method: poll.method ?? 'GET',
        headers: createRequestHeaders(profile, pollUrl),
        cache: 'no-store',
        signal,
      })
...
async function callResponsesImageApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    ...
    const response = await fetch(responsesUrl, {
      method: 'POST',
      headers: {
        ...createRequestHeaders(profile, responsesUrl),
        'Content-Type': 'application/json',
      },
      cache: 'no-store',
      body: JSON.stringify(body),
