<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      ...
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-[color:var(--bx-text-dim)]">
        {{ scanHint }}
      </p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-[color:var(--bx-text-dim)]">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-[color:var(--bx-text)]">{{ countdownDisplay }}</p>
        <p class="mt-2 text-sm text-[color:var(--bx-text-dim)]">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
