<template>
  <div class="bx-page flex min-h-screen items-center justify-center px-4">
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          ...
          <div v-else
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-red-900/30">
            <svg class="h-10 w-10 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <h2 class="mt-4 text-2xl font-bold text-[color:var(--bx-text)]">
            {{ statusTitle }}
          </h2>
          <p v-if="isPending" class="mt-2 text-sm text-[color:var(--bx-text-dim)]">
            {{ t('payment.result.processingHint') }}
          </p>
        </div>
        <!-- Order Info -->
        <div v-if="order" class="rounded-xl bg-[color:var(--bx-bg-elevated)] p-5 shadow-sm">
          <div class="space-y-3 text-sm">
            <div v-if="hasOrderId(order)" class="flex justify-between">
              ...
              <span class="font-medium text-[color:var(--bx-text)]">#{{ order.id }}</span>
            </div>
            <div v-if="order.out_trade_no" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.orderNo') }}</span>
              <span class="font-medium text-[color:var(--bx-text)]">{{ order.out_trade_no }}</span>
            </div>
            <div v-if="hasAmountFields(order)" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.baseAmount') }}</span>
              <span class="font-medium text-[color:var(--bx-text)]">{{ formatGatewayAmount(baseAmount) }}</span>
            </div>
            <div v-if="hasAmountFields(order) && order.fee_rate > 0" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</span>
              <span class="font-medium text-[color:var(--bx-text)]">{{ formatGatewayAmount(feeAmount) }}</span>
            </div>
            <div v-if="hasAmountFields(order)" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-bold text-primary-400">{{ formatGatewayAmount(order.pay_amount) }}</span>
            </div>
            <div v-if="hasAmountFields(order) && order.amount !== order.pay_amount" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.creditedAmount') }}</span>
              <span class="font-medium text-[color:var(--bx-text)]">{{ order.order_type === 'balance' ? '$' + order.amount.toFixed(2) : formatGatewayAmount(order.amount) }}</span>
            </div>
            <div v-if="hasPaymentType(order)" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.paymentMethod') }}</span>
              <span class="font-medium text-[color:var(--bx-text)]">{{ t(paymentMethodI18nKey(order.payment_type), normalizedOrderPaymentType(order.payment_type)) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.status') }}</span>
              <OrderStatusBadge :status="displayOrderStatus(order.status)" />
            </div>
          </div>
        </div>
        <!-- EasyPay return info (when no order loaded) -->
        <div v-else-if="returnInfo" class="rounded-xl bg-[color:var(--bx-bg-elevated)] p-5 shadow-sm">
          <div class="space-y-3 text-sm">
            <div v-if="returnInfo.outTradeNo" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.orderId') }}</span>
              <span class="font-medium text-[color:var(--bx-text)]">{{ returnInfo.outTradeNo }}</span>
            </div>
            <div v-if="returnInfo.money" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-medium text-[color:var(--bx-text)]">{{ formatGatewayAmount(Number(returnInfo.money) || 0) }}</span>
            </div>
            <div v-if="returnInfo.type" class="flex justify-between">
              <span class="text-[color:var(--bx-text-dim)]">{{ t('payment.orders.paymentMethod') }}</span>
              <span class="font-medium text-[color:var(--bx-text)]">{{ t(paymentMethodI18nKey(returnInfo.type), normalizedOrderPaymentType(returnInfo.type)) }}</span>
            </div>
          </div>
        </div>
        <!-- Actions -->
