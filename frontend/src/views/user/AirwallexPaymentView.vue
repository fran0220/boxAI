  ...
  <AppLayout>
    <div class="mx-auto max-w-lg space-y-6 py-8">
      ...
      <div v-else-if="errorMessage" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-[color:var(--bx-text)]">{{ t('payment.airwallexLoadFailed') }}</h3>
        <p class="mt-2 text-sm text-[color:var(--bx-text-dim)]">{{ errorMessage }}</p>
...
onMounted(async () => {
  ...
  try {
    ...
    const result = await airwallex.init({
      ...
      locale: checkoutLocale as 'en' | 'zh',
    })
