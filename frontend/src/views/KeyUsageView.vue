<template>
  <div class="bx-page relative flex min-h-screen flex-col">
    ...
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          ...
          <span class="text-lg font-semibold tracking-tight text-[color:var(--bx-text)]">{{ siteName }}</span>
        </router-link>
        <div class="flex items-center gap-3">
          ...
          <a
            ...
            class="rounded-lg p-2 text-[color:var(--bx-text-dim)] transition-colors hover:bg-[color:var(--bx-hover)] hover:text-gray-700   dark:hover:text-white"
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-3 text-[color:var(--bx-text)]">
          {{ t('keyUsage.title') }}
        </h1>
        <p class="text-[color:var(--bx-text-dim)] text-base max-w-md mx-auto">
          {{ t('keyUsage.subtitle') }}
        </p>
      </div>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            <div class="absolute left-4 top-1/2 -translate-y-1/2 text-[color:var(--bx-text-dim)]">
            ...
            <input
              ...
              class="input-ring w-full h-12 pl-12 pr-12 rounded-xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] text-sm text-[color:var(--bx-text)] placeholder:text-gray-400 transition-all    dark:placeholder:text-dark-500"
        ...
        <p class="text-xs text-[color:var(--bx-text-dim)] mt-3 text-center">
        ...
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-[color:var(--bx-text-dim)]">{{ t('keyUsage.dateRange') }}</span>
            <button
              ...
              :class="currentRange === range.key
                ...
                : 'border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] text-gray-700   dark:text-dark-200 hover:border-primary-300 dark:hover:border-dark-600'"
            >{{ range.label }}</button>
            <div v-if="currentRange === 'custom'" class="flex items-center gap-2 ml-1">
              <input
                v-model="customStartDate"
                type="date"
                class="input-ring text-xs px-2 py-1.5 rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] text-[color:var(--bx-text)]   "
              />
      ...
      <div v-if="showResults">
        ...
        <div v-if="showLoading" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="rounded-2xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-8  ">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] shadow-sm backdrop-blur-sm  ">
              <span
                class="w-2.5 h-2.5 rounded-full pulse-dot"
                :class="statusInfo.isActive ? 'bg-emerald-500' : 'bg-rose-500'"
              ></span>
              <span class="text-sm font-medium text-[color:var(--bx-text)]">{{ statusInfo.label }}</span>
              <span class="text-xs text-[color:var(--bx-text-dim)]">|</span>
              <span class="text-xs text-[color:var(--bx-text-dim)]">{{ statusInfo.statusText }}</span>
            </div>
          </div>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="fade-up rounded-2xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-8 backdrop-blur-sm transition-all duration-300 hover:shadow-lg  "
              :class="`fade-up-delay-${Math.min(i + 1, 4)}`"
            >
              <div class="flex items-center justify-between mb-6">
                <h3 class="text-sm font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">
                  {{ ring.title }}
                </h3>
                <!-- Clock icon -->
                <svg v-if="ring.iconType === 'clock'" class="w-5 h-5 text-[color:var(--bx-text-dim)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                <!-- Calendar icon -->
                <svg v-else-if="ring.iconType === 'calendar'" class="w-5 h-5 text-[color:var(--bx-text-dim)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                <!-- Dollar icon -->
                <svg v-else class="w-5 h-5 text-[color:var(--bx-text-dim)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-[color:var(--bx-text)]">
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span class="text-xs text-[color:var(--bx-text-dim)] mt-0.5">{{ t('keyUsage.used') }}</span>
                      <span
                        class="text-sm font-semibold mt-1 tabular-nums"
                        :style="{ color: RING_GRADIENTS[i % 4].from }"
                      >{{ ring.amount }}</span>
                      <p v-if="ring.resetAt && formatResetTime(ring.resetAt)" class="text-xs text-[color:var(--bx-text-dim)] mt-0.5 tabular-nums">
          ...
          <div
            ...
            class="fade-up fade-up-delay-3 rounded-2xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] backdrop-blur-sm overflow-hidden  "
          >
            <div class="px-8 py-5 border-b border-[color:var(--bx-border)]">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.detailInfo') }}</h3>
            </div>
            <div class="divide-y divide-[color:var(--bx-border)]">
              <div
              ...
              >
                ...
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-[color:var(--bx-text)]'">
                  {{ row.value }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="px-8 py-5 border-b border-[color:var(--bx-border)]">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.tokenStats') }}</h3>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-[color:var(--bx-bg-muted)]">
              <div
                v-for="(cell, i) in usageStatCells"
                :key="i"
                class="bg-[color:var(--bx-bg-elevated)] px-6 py-4 "
              >
                <div class="text-xs text-[color:var(--bx-text-dim)] mb-1">{{ cell.label }}</div>
                <div class="text-sm font-semibold tabular-nums text-[color:var(--bx-text)]">{{ cell.value }}</div>
              </div>
            </div>
          </div>
          ...
          <div
            v-if="showDailyUsage"
            class="fade-up fade-up-delay-4 rounded-2xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] backdrop-blur-sm overflow-hidden  "
          >
            <div class="flex flex-col gap-3 px-8 py-5 border-b border-[color:var(--bx-border)] sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.dailyDetail') }}</h3>
              <div class="inline-flex rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-0.5  ">
                <button
                  ...
                  :class="dailyUsageDays === option.value
                    ...
                    : 'text-gray-600 hover:bg-[color:var(--bx-hover)] dark:text-dark-300 '"
                >
                  {{ option.label }}
                </button>
              </div>
            ...
            <div v-if="dailyUsageRows.length > 0" class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)]  ">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.date') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.cacheWriteTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    :key="row.date"
                    class="border-b border-[color:var(--bx-border)] last:border-b-0 "
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-[color:var(--bx-text)]">{{ row.date }}</td>
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-[color:var(--bx-text)]">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="px-8 py-8 text-center text-sm text-[color:var(--bx-text-dim)]">
          ...
          >
            <div class="px-8 py-5 border-b border-[color:var(--bx-border)]">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.modelStats') }}</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)]  ">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.model') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class="border-b border-[color:var(--bx-border)] last:border-b-0 "
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-[color:var(--bx-text)]">{{ m.model || '-' }}</td>
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-[color:var(--bx-text)]">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <footer class="relative z-10 border-t border-[color:var(--bx-border)] px-6 py-8 /50">
      <div class="mx-auto flex max-w-6xl flex-col items-center justify-center gap-4 text-center sm:flex-row sm:text-left">
        ...
        <div class="flex items-center gap-4">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            class="text-sm text-[color:var(--bx-text-dim)] transition-colors hover:text-gray-700  dark:hover:text-white"
          >{{ t('home.docs') }}</a>
          <a
            :href="githubUrl"
            target="_blank"
...
function formatDate(iso: string | null | undefined): string {
  ...
  const loc = localeToBcp47(String(locale.value))
