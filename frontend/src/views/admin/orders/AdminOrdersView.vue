      ...
      <OrderTable :orders="orders" :loading="ordersLoading" show-user>
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            <button @click="showOrderDetail(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-[color:var(--bx-text-muted)] hover:bg-[color:var(--bx-hover)]  ">
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.orders.orderId') }}</p><p class="font-mono text-sm font-medium text-[color:var(--bx-text)]">#{{ selectedOrder.id }}</p></div>
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.orders.orderNo') }}</p><p class="text-sm font-medium text-[color:var(--bx-text)]">{{ selectedOrder.out_trade_no }}</p></div>
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.orders.status') }}</p><OrderStatusBadge :status="selectedOrder.status" /></div>
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.orders.amount') }}</p><p class="text-sm font-medium text-[color:var(--bx-text)]">{{ creditedAmountSymbol }}{{ selectedOrder.amount.toFixed(2) }}</p></div>
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.orders.payAmount') }}</p><p class="text-sm font-medium text-[color:var(--bx-text)]">{{ paymentAmountSymbol(selectedOrder) }}{{ selectedOrder.pay_amount.toFixed(2) }}</p></div>
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.orders.paymentMethod') }}</p><p class="text-sm text-[color:var(--bx-text-soft)]">{{ t('payment.methods.' + selectedOrder.payment_type, selectedOrder.payment_type) }}</p></div>
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.admin.feeRate') }}</p><p class="text-sm text-[color:var(--bx-text-soft)]">{{ selectedOrder.fee_rate }}%</p></div>
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.orders.createdAt') }}</p><p class="text-sm text-[color:var(--bx-text-soft)]">{{ formatDateTime(selectedOrder.created_at) }}</p></div>
          <div><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.admin.expiresAt') }}</p><p class="text-sm text-[color:var(--bx-text-soft)]">{{ formatDateTime(selectedOrder.expires_at) }}</p></div>
          <div v-if="selectedOrder.paid_at"><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.admin.paidAt') }}</p><p class="text-sm text-[color:var(--bx-text-soft)]">{{ formatDateTime(selectedOrder.paid_at) }}</p></div>
          <div v-if="selectedOrder.refund_amount"><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.admin.refundAmount') }}</p><p class="text-sm font-medium text-red-600 dark:text-red-400">{{ creditedAmountSymbol }}{{ selectedOrder.refund_amount.toFixed(2) }}</p></div>
          <div v-if="selectedOrder.refund_reason" class="col-span-2"><p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.admin.refundReason') }}</p><p class="text-sm text-[color:var(--bx-text-soft)]">{{ selectedOrder.refund_reason }}</p></div>
          <!-- Refund request info -->
          <div v-if="selectedOrder.refund_requested_at" class="col-span-2 border-t border-[color:var(--bx-border)] pt-3 ">
            <p class="mb-2 text-xs font-medium text-purple-600 dark:text-purple-400">{{ t('payment.admin.refundRequestInfo') }}</p>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.admin.refundRequestedAt') }}</p>
                <p class="text-sm text-[color:var(--bx-text-soft)]">{{ formatDateTime(selectedOrder.refund_requested_at) }}</p>
              </div>
              <div>
                <p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.admin.refundRequestedBy') }}</p>
                <p class="text-sm text-[color:var(--bx-text-soft)]">#{{ selectedOrder.refund_requested_by }}</p>
              </div>
              <div class="col-span-2">
                <p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('payment.admin.refundRequestReason') }}</p>
                <p class="text-sm text-[color:var(--bx-text-soft)]">{{ selectedOrder.refund_request_reason }}</p>
              </div>
            </div>
          </div>
        </div>
        <!-- Audit Logs -->
        <div v-if="orderAuditLogs.length > 0" class="border-t border-[color:var(--bx-border)] pt-4 ">
          <p class="mb-2 text-xs font-medium text-[color:var(--bx-text-dim)]">{{ t('payment.admin.auditLogs') }}</p>
          <div class="max-h-48 space-y-2 overflow-y-auto">
            <div v-for="log in orderAuditLogs" :key="log.id" class="rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)] p-2.5  ">
              <div class="flex items-center justify-between">
                <span class="text-xs font-medium text-[color:var(--bx-text-soft)]">{{ log.action }}</span>
                <span class="text-xs text-gray-400">{{ formatDateTime(log.created_at) }}</span>
              </div>
              <div v-if="log.detail" class="mt-1 break-all text-xs text-[color:var(--bx-text-dim)]">{{ log.detail }}</div>
