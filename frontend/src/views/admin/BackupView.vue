    ...
    <div class="space-y-6">
      ...
      <div class="card p-6">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.s3.endpoint') }}</label>
            <input v-model="s3Form.endpoint" class="input w-full" placeholder="https://<account_id>.r2.cloudflarestorage.com" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.s3.region') }}</label>
            <input v-model="s3Form.region" class="input w-full" placeholder="auto" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.s3.bucket') }}</label>
            <input v-model="s3Form.bucket" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.s3.prefix') }}</label>
            <input v-model="s3Form.prefix" class="input w-full" placeholder="backups/" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.s3.accessKeyId') }}</label>
            <input v-model="s3Form.access_key_id" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.s3.secretAccessKey') }}</label>
          ...
          <label class="inline-flex items-center gap-2 text-sm text-[color:var(--bx-text-soft)] md:col-span-2">
      ...
      <div class="card p-6">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.schedule.cronExpr') }}</label>
            <input v-model="scheduleForm.cron_expr" class="input w-full" placeholder="0 2 * * *" />
            <p class="mt-1 text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.backup.schedule.cronHint') }}</p>
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.schedule.retainDays') }}</label>
            <input v-model.number="scheduleForm.retain_days" type="number" min="0" class="input w-full" />
            <p class="mt-1 text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.backup.schedule.retainDaysHint') }}</p>
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.backup.schedule.retainCount') }}</label>
            <input v-model.number="scheduleForm.retain_count" type="number" min="0" class="input w-full" />
            <p class="mt-1 text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.backup.schedule.retainCountHint') }}</p>
          </div>
        </div>
        <div class="mt-4">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <div class="flex items-center gap-1">
              <label class="text-xs text-[color:var(--bx-text-muted)]">{{ t('admin.backup.operations.expireDays') }}</label>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[800px] text-sm">
            <thead>
              <tr class="border-b border-[color:var(--bx-border)] text-left text-xs uppercase tracking-wide text-[color:var(--bx-text-dim)]  ">
            ...
            <tbody>
              <tr v-for="record in backups" :key="record.id" class="border-b border-[color:var(--bx-border)] align-top ">
              ...
              <tr v-if="backups.length === 0">
                <td colspan="8" class="py-6 text-center text-sm text-[color:var(--bx-text-dim)]">
    ...
    <teleport to="body">
      <transition name="modal">
        <div v-if="showR2Guide" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showR2Guide = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-[color:var(--bx-bg-elevated)] p-6 shadow-2xl">
            <button type="button" class="absolute right-4 top-4 text-gray-400 hover:text-[color:var(--bx-text)]" @click="showR2Guide = false">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <h2 class="mb-4 text-lg font-bold text-[color:var(--bx-text)]">{{ t('admin.backup.r2Guide.title') }}</h2>
            <p class="mb-4 text-sm text-[color:var(--bx-text-dim)]">{{ t('admin.backup.r2Guide.intro') }}</p>

            <!-- Step 1 -->
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-[color:var(--bx-text)]">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-primary-100 text-xs font-bold text-primary-700 dark:bg-primary-900/40 dark:text-primary-300">1</span>
                {{ t('admin.backup.r2Guide.step1.title') }}
              </h3>
              <ol class="ml-8 list-decimal space-y-1 text-sm text-[color:var(--bx-text-muted)]">
            ...
            <div class="mb-5">
              ...
              <p class="ml-8 text-sm text-[color:var(--bx-text-muted)]">{{ t('admin.backup.r2Guide.step3.desc') }}</p>
              <code class="ml-8 mt-1 block rounded bg-[color:var(--bx-bg-muted)] px-3 py-2 text-xs text-gray-800  dark:text-gray-200">https://&lt;{{ t('admin.backup.r2Guide.step3.accountId') }}&gt;.r2.cloudflarestorage.com</code>
            </div>
            ...
            <div class="mb-5">
              ...
              <div class="ml-8 overflow-hidden rounded-lg border border-[color:var(--bx-border)]">
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in r2ConfigRows" :key="i" class="border-b border-[color:var(--bx-border)] last:border-0">
                      <td class="whitespace-nowrap bg-[color:var(--bx-bg-muted)] px-3 py-2 font-medium text-[color:var(--bx-text-soft)]  ">{{ row.field }}</td>
                      <td class="px-3 py-2 text-[color:var(--bx-text-muted)]"><code class="text-xs">{{ row.value }}</code></td>
                    </tr>
                  </tbody>
                </table>
              </div>
