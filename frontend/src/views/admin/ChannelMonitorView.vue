      ...
      <template #table>
        <DataTable :columns="columns" :data="monitors" :loading="loading">
          ...
          <template #cell-availability_7d="{ row }">
            <span class="text-sm text-[color:var(--bx-text)]">{{ formatAvailability(row) }}</span>
          </template>

          <template #cell-latency="{ row }">
            <span class="text-sm text-[color:var(--bx-text)]">{{ formatLatency(row.primary_latency_ms) }}</span>
          </template>
