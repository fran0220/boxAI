...
<template>
  <div class="flex h-full flex-col rounded-3xl bg-[color:var(--bx-bg-elevated)] p-6 shadow-sm ring-1 ring-gray-900/5  dark:ring-dark-700">
    <!-- 头部 -->
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      <h3 class="flex items-center gap-2 text-sm font-bold text-[color:var(--bx-text)]">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="showByUser
            ...
            : 'bg-[color:var(--bx-bg-muted)] text-[color:var(--bx-text-dim)] hover:bg-gray-200 hover:text-gray-700   dark:hover:bg-dark-600 dark:hover:text-gray-300'"
        ...
        <!-- 刷新按钮 -->
        <button
          class="flex items-center gap-1 rounded-lg bg-[color:var(--bx-bg-muted)] px-2 py-1 text-[11px] font-semibold text-[color:var(--bx-text-soft)] transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50   dark:hover:bg-dark-600"
          :disabled="loading"
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-[color:var(--bx-border)] text-sm text-[color:var(--bx-text-dim)]  "
    >
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[color:var(--bx-border)]">
      <!-- 维度标题栏 -->
      <div class="flex shrink-0 items-center justify-between border-b border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)] px-3 py-2  ">
        <span class="text-[10px] font-bold uppercase tracking-wider text-[color:var(--bx-text-dim)]">
          {{ displayTitle }}
        </span>
        <span class="text-[10px] text-[color:var(--bx-text-dim)]">
          {{ t('admin.ops.concurrency.totalRows', { count: displayRows.length }) }}
        </span>
      </div>

      <!-- 空状态 -->
      <div v-if="displayRows.length === 0" class="flex flex-1 items-center justify-center text-sm text-[color:var(--bx-text-dim)]">
      ...
      <div v-else-if="displayDimension === 'user'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-[color:var(--bx-bg-muted)] p-2.5 ">
          <!-- 用户信息和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate text-[11px] font-bold text-[color:var(--bx-text)]" :title="row.username || row.user_email">
                {{ row.username || row.user_email }}
              </span>
              <span v-if="row.username" class="shrink-0 truncate text-[10px] text-[color:var(--bx-text-dim)]" :title="row.user_email">
                {{ row.user_email }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-[color:var(--bx-text)]"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
      ...
      <div v-else-if="displayDimension === 'platform' || displayDimension === 'group'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as SummaryRow[])" :key="row.key" class="rounded-lg bg-[color:var(--bx-bg-muted)] p-3 ">
          <!-- 标题行 -->
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <div class="truncate text-[11px] font-bold text-[color:var(--bx-text)]" :title="row.name">
                {{ row.name }}
              </div>
              <span v-if="displayDimension === 'group' && row.platform" class="text-[10px] text-[color:var(--bx-text-dim)]">
                {{ row.platform.toUpperCase() }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-[color:var(--bx-text)]"> {{ row.used_concurrency }}/{{ row.total_concurrency }} </span>
              <span :class="['font-bold', getLoadTextClass(row.concurrency_percentage)]"> {{ row.concurrency_percentage }}% </span>
            </div>
          </div>

          <!-- 进度条 -->
          <div class="mb-2 h-1.5 w-full overflow-hidden rounded-full bg-[color:var(--bx-bg-muted)]">
            <div
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              ...
              <span class="text-[color:var(--bx-text-dim)]">{{ row.availability_percentage }}%</span>
            </div>
      ...
      <div v-else class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as AccountRow[])" :key="row.key" class="rounded-lg bg-[color:var(--bx-bg-muted)] p-2.5 ">
          <!-- 账号名称和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 text-[9px] text-[color:var(--bx-text-dim)]">
                {{ row.group_name }}
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-bold text-[color:var(--bx-text)]"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <!-- 状态徽章 -->
              <span
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-700 dark:bg-[color:var(--bx-bg-elevated)] dark:text-gray-400"
              >
