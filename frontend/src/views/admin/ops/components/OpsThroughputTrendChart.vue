  ...
  <div class="flex h-full flex-col rounded-3xl bg-[color:var(--bx-bg-elevated)] p-6 shadow-sm ring-1 ring-gray-900/5  dark:ring-dark-700">
    <div class="mb-4 flex shrink-0 items-center justify-between">
      ...
      <div class="flex items-center gap-2 text-xs text-[color:var(--bx-text-dim)]">
        ...
        <template v-if="!props.fullscreen">
          <button
            type="button"
            class="ml-2 inline-flex items-center rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] px-2 py-1 text-[11px] font-semibold text-[color:var(--bx-text-muted)] hover:bg-[color:var(--bx-hover)] disabled:opacity-50    "
          ...
          <button
            type="button"
            class="inline-flex items-center rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] px-2 py-1 text-[11px] font-semibold text-[color:var(--bx-text-muted)] hover:bg-[color:var(--bx-hover)] disabled:opacity-50    "
            :disabled="state !== 'ready'"
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      <button
        ...
        :key="g.group_id"
        type="button"
        class="inline-flex items-center gap-2 rounded-full border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] px-3 py-1 text-[11px] font-semibold text-[color:var(--bx-text-soft)] hover:bg-[color:var(--bx-hover)]    "
        @click="emit('selectGroup', g.group_id)"
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_id}` }}</span>
        <span class="text-[color:var(--bx-text-dim)]">{{ formatNumber(g.request_count) }}</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="text-[color:var(--bx-text-dim)]">{{ formatNumber(p.request_count) }}</span>
      </button>
    </div>
