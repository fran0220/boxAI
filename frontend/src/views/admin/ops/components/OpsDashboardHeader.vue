...
const cpuPercentClass = computed(() => {
  ...
  if (v == null) return 'text-[color:var(--bx-text)]'
...
<template>
  <div :class="['flex flex-col gap-4 rounded-3xl bg-[color:var(--bx-bg-elevated)] shadow-sm ring-1 ring-gray-900/5  dark:ring-dark-700', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-[color:var(--bx-border)] pb-4 ">
      <div>
        <h1 class="flex items-center gap-2 text-xl font-black text-[color:var(--bx-text)]">
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-[color:var(--bx-text-dim)]">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-[color:var(--bx-bg-muted)] sm:block"></div>
        ...
        <button
          ...
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 text-[color:var(--bx-text-dim)] transition-colors hover:bg-[color:var(--bx-bg-muted)]  dark:hover:bg-dark-600"
          :disabled="loading"
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-[color:var(--bx-bg-muted)] sm:block"></div>
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-gray-100 px-3 text-xs font-bold text-[color:var(--bx-text-soft)] transition-colors hover:bg-[color:var(--bx-bg-muted)]  dark:hover:bg-dark-600"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 text-[color:var(--bx-text-soft)] transition-colors hover:bg-[color:var(--bx-bg-muted)]  dark:hover:bg-dark-600"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-[color:var(--bx-bg-muted)] lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          ...
          >
            ...
            >
              <div class="rounded-xl bg-white p-4 shadow-xl ring-1 ring-black/5 dark:bg-[color:var(--bx-bg-elevated)] dark:ring-white/10">
                <h4 class="mb-3 border-b border-gray-100 pb-2 text-sm font-bold text-[color:var(--bx-text)] dark:border-[color:var(--bx-border)]  flex items-center gap-2">
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-[color:var(--bx-text)]">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-[color:var(--bx-text-dim)]">{{ item.impact }}</div>
                ...
                <div class="mt-3 border-t border-gray-100 pt-2 text-[10px] text-gray-400 dark:border-[color:var(--bx-border)]">
              ...
              <div class="flex flex-wrap gap-1">
                <button
                  ...
                  :class="realtimeWindow === window
                    ...
                    : 'bg-gray-200 text-[color:var(--bx-text-muted)] hover:bg-gray-300 dark:bg-dark-700  dark:hover:bg-dark-600'"
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              ...
              <div>
                ...
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-[color:var(--bx-text)]']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-gray-500']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-[color:var(--bx-text)]']">{{ displayRealTimeTps.toFixed(1) }}</span>
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-[color:var(--bx-text-muted)]']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-[color:var(--bx-text)]">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-[color:var(--bx-text)]">{{ realtimeTpsPeakLabel }}</span>
                ...
                <div>
                  ...
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-[color:var(--bx-text-muted)]']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-[color:var(--bx-text)]">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-[color:var(--bx-text)]">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 " style="order: 1;">
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              ...
              <span class="font-bold text-[color:var(--bx-text)]">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 " style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-[color:var(--bx-bg-muted)]">
        ...
        <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 " style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-black text-[color:var(--bx-text)]">
              {{ durationP99Ms ?? '-' }}
            </div>
          ...
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              ...
              <span class="font-bold text-[color:var(--bx-text)]">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P90:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P50:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">Avg:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">Max:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 " style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
        ...
        <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 " style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              ...
              <span class="font-bold text-[color:var(--bx-text)]">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 " style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              ...
              <span class="font-bold text-[color:var(--bx-text)]">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">429/529:</span>
              <span class="font-bold text-[color:var(--bx-text)]">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-[color:var(--bx-border)] pt-4 ">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="rounded-xl bg-[color:var(--bx-bg-muted)] p-3 ">
          <div class="flex items-center gap-1">
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-[color:var(--bx-text-dim)]">
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-[color:var(--bx-text-dim)]">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        ...
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-[color:var(--bx-text)]">{{ hb.job_name }}</div>
            <div class="flex items-center gap-3 text-xs text-[color:var(--bx-text-dim)]">
              <span v-if="hb.last_duration_ms != null" class="font-mono">{{ hb.last_duration_ms }}ms</span>
              <span>{{ formatTimeShort(hb.updated_at) }}</span>
            </div>
          </div>

          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-[color:var(--bx-text-muted)] sm:grid-cols-2">
            <div>
    ...
    <BaseDialog :show="showCustomTimeRangeDialog" :title="t('admin.ops.timeRange.custom')" width="narrow" @close="handleCustomTimeRangeCancel">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-[color:var(--bx-text-soft)] mb-1">
            {{ t('admin.ops.customTimeRange.startTime') }}
          </label>
          <input
            v-model="customStartTimeInput"
            type="datetime-local"
            class="w-full rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] px-3 py-2 text-sm text-[color:var(--bx-text)] focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500   "
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-[color:var(--bx-text-soft)] mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-[color:var(--bx-text-soft)] hover:bg-[color:var(--bx-bg-muted)]  dark:hover:bg-dark-600"
