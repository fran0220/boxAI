...
<template>
  <div class="rounded-3xl bg-[color:var(--bx-bg-elevated)] p-6 shadow-sm ring-1 ring-gray-900/5  dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-[color:var(--bx-text)]">{{ t('admin.ops.email.title') }}</h3>
        <p class="mt-1 text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.ops.email.description') }}</p>
      </div>
    ...
    <div v-if="!config" class="text-sm text-[color:var(--bx-text-dim)]">
    ...
    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 ">
        <h4 class="mb-2 text-sm font-semibold text-[color:var(--bx-text)]">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-[color:var(--bx-text-muted)]">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-[color:var(--bx-text)]">
              {{ config.alert.enabled ? t('common.enabled') : t('common.disabled') }}
            </span>
          </div>
          <div class="text-xs text-[color:var(--bx-text-muted)]">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-[color:var(--bx-text)]">{{ config.alert.recipients.length }}</span>
          </div>
          <div class="text-xs text-[color:var(--bx-text-muted)]">
          ...
          <div class="text-xs text-[color:var(--bx-text-muted)]">
            ...
            <span class="ml-1 font-medium text-[color:var(--bx-text)]">{{ config.alert.rate_limit_per_hour }}</span>
          </div>
        </div>
      </div>

      <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 ">
        <h4 class="mb-2 text-sm font-semibold text-[color:var(--bx-text)]">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-[color:var(--bx-text-muted)]">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-[color:var(--bx-text)]">
          ...
          <div class="text-xs text-[color:var(--bx-text-muted)]">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-[color:var(--bx-text)]">{{ config.report.recipients.length }}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <BaseDialog :show="showEditor" :title="t('admin.ops.email.title')" width="extra-wide" @close="showEditor = false">
    <div v-if="draft" class="space-y-6">
      ...
      <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 ">
        <h4 class="mb-3 text-sm font-semibold text-[color:var(--bx-text)]">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('common.enabled') }}</div>
          ...
          <div>
            <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.minSeverity') }}</div>
            <Select v-model="draft.alert.min_severity" :options="severityOptions" />
          </div>

          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.recipients') }}</div>
            ...
            <div class="mt-1 text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.ops.email.recipientsHint') }}</div>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.rateLimitPerHour') }}</div>
            <input v-model.number="draft.alert.rate_limit_per_hour" type="number" min="0" max="100000" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.batchWindowSeconds') }}</div>
            <input v-model.number="draft.alert.batching_window_seconds" type="number" min="0" max="86400" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.includeResolved') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-[color:var(--bx-text-soft)]">
      ...
      <div class="rounded-2xl bg-[color:var(--bx-bg-muted)] p-4 ">
        <h4 class="mb-3 text-sm font-semibold text-[color:var(--bx-text)]">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-[color:var(--bx-text-soft)]">
          ...
          <div class="md:col-span-2">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.dailySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-[color:var(--bx-text-soft)]">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.weeklySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-[color:var(--bx-text-soft)]">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.errorDigest') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-[color:var(--bx-text-soft)]">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.errorDigestMinCount') }}</div>
                <input v-model.number="draft.report.error_digest_min_count" type="number" min="0" max="1000000" class="input" />
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.accountHealth') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-[color:var(--bx-text-soft)]">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.ops.email.accountHealthThreshold') }}</div>
                <input v-model.number="draft.report.account_health_error_rate_threshold" type="number" min="0" max="100" step="0.1" class="input" />
              </div>
            </div>
            <div class="mt-2 text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.ops.email.reportHint') }}</div>
          </div>
        </div>
      </div>
    </div>
