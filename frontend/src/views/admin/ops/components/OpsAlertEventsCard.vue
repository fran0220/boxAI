...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p3') return 'bg-[color:var(--bx-bg-muted)] text-[color:var(--bx-text-soft)]  '
  return 'bg-[color:var(--bx-bg-muted)] text-[color:var(--bx-text-soft)]  '
}
...
function statusBadgeClass(status: string | undefined): string {
  ...
  return 'bg-gray-50 text-[color:var(--bx-text-soft)] ring-gray-600/20 dark:bg-[color:var(--bx-bg-elevated)]  dark:ring-gray-500/30'
}
...
<template>
  <div class="rounded-3xl bg-[color:var(--bx-bg-elevated)] p-6 shadow-sm ring-1 ring-gray-900/5  dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-[color:var(--bx-text)]">{{ t('admin.ops.alertEvents.title') }}</h3>
        <p class="mt-1 text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-[color:var(--bx-text-dim)]">
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-[color:var(--bx-border)] p-8 text-center text-sm text-[color:var(--bx-text-dim)]  ">
      {{ t('admin.ops.alertEvents.empty') }}
    </div>

    <div v-else class="overflow-hidden rounded-xl border border-[color:var(--bx-border)]">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <table class="min-w-full divide-y divide-[color:var(--bx-border)]">
          ...
          <tbody class="divide-y divide-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)]">
            ...
            >
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-[color:var(--bx-text-soft)]">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-[color:var(--bx-text-dim)]">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  ...
                  <Icon
                    ...
                    name="ban"
                    size="sm"
                    class="text-[color:var(--bx-text-dim)]"
                  />
                  <span class="text-[11px] font-bold text-[color:var(--bx-text-muted)]">
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-[color:var(--bx-text-dim)]">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-[color:var(--bx-text-dim)]">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-[color:var(--bx-text-dim)]">
      ...
      <div v-else class="space-y-5">
        <div class="rounded-xl bg-[color:var(--bx-bg-muted)] p-4 ">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div class="mt-2 text-sm font-semibold text-[color:var(--bx-text)]">
                {{ selected.title || '-' }}
              </div>
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-[color:var(--bx-text-muted)]">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-[color:var(--bx-bg-elevated)] px-2 py-1 ring-1 ring-gray-200  dark:ring-dark-700">
                <span class="text-[11px] font-bold text-[color:var(--bx-text-muted)]">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-[color:var(--bx-bg-muted)] p-4 ">
              ...
              <div class="mt-1 text-sm font-medium text-[color:var(--bx-text)]">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-[color:var(--bx-bg-muted)] p-4 ">
              <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-[color:var(--bx-text)]">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-[color:var(--bx-bg-muted)] p-4 ">
              <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-[color:var(--bx-text)]">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-bold text-[color:var(--bx-text-soft)] ring-1 ring-gray-200 hover:bg-[color:var(--bx-bg-muted)]  dark:ring-dark-700 dark:hover:bg-dark-700"
            ...
            <div class="rounded-xl bg-[color:var(--bx-bg-muted)] p-4 ">
              ...
              <div class="mt-1 text-sm text-[color:var(--bx-text)]">
        ...
        <div class="rounded-xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-4  ">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-[color:var(--bx-text)]">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-[color:var(--bx-text-dim)]">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-[color:var(--bx-text-dim)]">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-[color:var(--bx-border)]">
            <table class="min-w-full divide-y divide-gray-100 dark:divide-dark-700">
              <thead class="bg-[color:var(--bx-bg-muted)]">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-[color:var(--bx-text-dim)]">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
                <tr v-for="it in history" :key="it.id" class="hover:bg-[color:var(--bx-hover)]">
                  <td class="px-3 py-2 text-xs text-[color:var(--bx-text-muted)]">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
                  ...
                  <td class="px-3 py-2 text-xs text-[color:var(--bx-text-muted)]">
