    ...
    <div class="card flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <template #cell-model="{ row }">
          <span v-if="row.model" class="text-sm font-medium text-[color:var(--bx-text)]">{{ row.model }}</span>
          <span v-else class="text-sm text-[color:var(--bx-text-dim)]">-</span>
        </template>

        <template #cell-key_name="{ row }">
          <div class="text-sm">
            <span class="text-[color:var(--bx-text)]">{{ row.key_name || '-' }}</span>
            <span
        ...
        <template #cell-category="{ row }">
          <span class="text-sm text-[color:var(--bx-text)]">{{ t('usage.errors.categories.' + row.category) }}</span>
        </template>
        ...
        <template #cell-created_at="{ row }">
          <span class="text-sm text-[color:var(--bx-text-muted)]">{{ formatDateTime(row.created_at) }}</span>
        </template>
