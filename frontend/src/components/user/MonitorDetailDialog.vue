  ...
  >
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full text-left text-sm">
        <thead class="border-b border-[color:var(--bx-border)]">
          <tr class="text-xs uppercase tracking-wider text-[color:var(--bx-text-dim)]">
        ...
        <tbody>
          <tr
            ...
            :key="m.model"
            class="border-b border-[color:var(--bx-border)]"
          >
            <td class="py-2 pr-3 font-medium text-[color:var(--bx-text)]">{{ m.model }}</td>
            <td class="py-2 pr-3">
              <span
            ...
            <td class="py-2 pr-3 text-[color:var(--bx-text-soft)]">{{ formatLatency(m.latest_latency_ms) }}</td>
            <td class="py-2 pr-3 text-[color:var(--bx-text-soft)]">{{ formatPercent(m.availability_7d) }}</td>
            <td class="py-2 pr-3 text-[color:var(--bx-text-soft)]">{{ formatPercent(m.availability_15d) }}</td>
            <td class="py-2 pr-3 text-[color:var(--bx-text-soft)]">{{ formatPercent(m.availability_30d) }}</td>
            <td class="py-2 pr-3 text-[color:var(--bx-text-soft)]">{{ formatLatency(m.avg_latency_7d_ms) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
