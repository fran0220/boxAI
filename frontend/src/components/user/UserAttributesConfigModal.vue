    ...
    <div class="space-y-4">
      ...
      <div v-else class="max-h-96 space-y-2 overflow-y-auto">
        <div
          ...
          :key="attr.id"
          class="flex items-center gap-3 rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-3  "
        >
          <!-- Drag Handle -->
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-[color:var(--bx-text)]">{{ attr.name }}</span>
              <span class="rounded bg-[color:var(--bx-bg-muted)] px-1.5 py-0.5 font-mono text-xs text-[color:var(--bx-text-dim)]  ">
                {{ attr.key }}
              </span>
            ...
            <div class="mt-0.5 flex items-center gap-2 text-xs text-[color:var(--bx-text-dim)]">
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="rounded-lg p-1.5 text-gray-500 hover:bg-[color:var(--bx-hover)] hover:text-primary-600  dark:hover:text-primary-400"
              :title="t('common.edit')"
            >
    ...
    <form id="attribute-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2">
          ...
          <span class="text-sm text-[color:var(--bx-text-soft)]">{{ t('admin.users.attributes.required') }}</span>
        </label>
        <label class="flex items-center gap-2">
          <input v-model="form.enabled" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-primary-600" />
          <span class="text-sm text-[color:var(--bx-text-soft)]">{{ t('admin.users.attributes.enabled') }}</span>
        </label>
      </div>
    </form>
