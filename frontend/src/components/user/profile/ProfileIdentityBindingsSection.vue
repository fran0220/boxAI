<template>
  <div :class="props.embedded ? 'space-y-4' : 'card overflow-hidden'">
    ...
    >
      <h2 class="text-lg font-medium text-[color:var(--bx-text)]">
    ...
    <div :class="props.embedded ? 'space-y-4' : 'divide-y divide-gray-100 dark:divide-dark-700'">
      <div v-if="props.embedded">
        <p class="text-sm font-semibold text-[color:var(--bx-text)]">
          {{ t('profile.authBindings.title') }}
        </p>
        <p class="mt-1 text-sm text-[color:var(--bx-text-dim)]">
          {{ t('profile.authBindings.description') }}
          ...
          <div class="flex min-w-0 flex-1 items-start gap-4">
            ...
            <div class="min-w-0 flex-1 space-y-3">
              ...
              <div
                ...
                class="grid gap-1 text-sm text-[color:var(--bx-text-dim)]"
              >
                <p
                  v-if="item.provider !== 'email' && item.details?.display_name"
                  class="font-medium text-[color:var(--bx-text-soft)]"
                >
...
const rowClass = computed(() =>
  props.embedded
    ? compact.value
      ? 'rounded-2xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-4 shadow-sm  '
      : 'rounded-2xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)] p-4  '
    : 'px-6 py-5'
)
