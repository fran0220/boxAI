<template>
  <span v-if="props.quotas === undefined" class="text-xs text-[color:var(--bx-text-dim)]">…</span>
  <span v-else-if="configured.length === 0" class="text-xs text-[color:var(--bx-text-dim)]">
  ...
  <div v-else class="space-y-0.5 text-xs">
    ...
    >
      <span class="w-20 shrink-0 font-mono text-[color:var(--bx-text-soft)]">{{ row.platform }}</span>
      <span class="text-[color:var(--bx-text-dim)]">
        {{ t('admin.users.platformQuota.windowDaily') }}
        <span class="text-[color:var(--bx-text)]">{{ fmtUsd(row.daily_usage_usd) }}/{{ fmtLimit(row.daily_limit_usd) }}</span>
      </span>
      <span class="text-[color:var(--bx-text-dim)]">
        {{ t('admin.users.platformQuota.windowWeekly') }}
        <span class="text-[color:var(--bx-text)]">{{ fmtUsd(row.weekly_usage_usd) }}/{{ fmtLimit(row.weekly_limit_usd) }}</span>
      </span>
      <span class="text-[color:var(--bx-text-dim)]">
        {{ t('admin.users.platformQuota.windowMonthly') }}
        <span class="text-[color:var(--bx-text)]">{{ fmtUsd(row.monthly_usage_usd) }}/{{ fmtLimit(row.monthly_limit_usd) }}</span>
      </span>
    </div>
  </div>
</template>
