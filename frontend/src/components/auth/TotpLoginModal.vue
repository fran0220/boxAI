<template>
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-md transform rounded-xl bg-[color:var(--bx-bg-elevated)] p-6 shadow-xl transition-all ">
        <!-- Header -->
        <div class="mb-6 text-center">
          <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-primary-900/30">
            <svg class="h-6 w-6 text-primary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
            </svg>
          </div>
          <h3 class="mt-4 text-xl font-semibold text-[color:var(--bx-text)]">
            {{ t('profile.totp.loginTitle') }}
          </h3>
          <p class="mt-2 text-sm text-[color:var(--bx-text-dim)]">
            {{ t('profile.totp.loginHint') }}
          </p>
          <p v-if="userEmailMasked" class="mt-1 text-sm font-medium text-[color:var(--bx-text-soft)]">
            {{ userEmailMasked }}
          </p>
        </div>
        ...
        <div class="mb-6">
          ...
          <div class="flex justify-center gap-2">
            <input
              ...
              pattern="[0-9]"
              autocomplete="off"
              class="h-12 w-10 rounded-lg border border-[color:var(--bx-border)] text-center text-lg font-semibold focus:border-primary-500 focus:ring-primary-500  dark:bg-dark-700"
              :disabled="verifying"
