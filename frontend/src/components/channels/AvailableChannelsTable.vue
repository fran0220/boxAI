<template>
  <div class="card overflow-hidden">
    <table class="w-full table-fixed border-collapse text-sm">
      <thead>
        <tr class="border-b border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)] text-xs font-medium uppercase tracking-wide text-[color:var(--bx-text-dim)]   ">
      ...
      <tbody v-else-if="rows.length === 0">
        <tr>
          <td colspan="5" class="py-12 text-center">
            ...
            <p class="text-sm text-[color:var(--bx-text-dim)]">{{ emptyLabel }}</p>
          </td>
        </tr>
      </tbody>
      ...
      <tbody
        ...
        class="border-b-2 border-[color:var(--bx-border)] last:border-b-0 "
      >
        <tr
          v-for="(section, secIdx) in channel.platforms"
          :key="`${channel.name}-${section.platform}`"
          class="transition-colors hover:bg-[color:var(--bx-hover)] "
          :class="{ 'border-t border-[color:var(--bx-border)] /50': secIdx > 0 }"
        >
          <!-- 渠道名：只在第一行渲染并用 rowspan 纵向合并 -->
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 text-center align-middle font-medium text-[color:var(--bx-text)]"
          >
            {{ channel.name }}
          </td>
          ...
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 align-middle text-xs text-[color:var(--bx-text-dim)]"
          >
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-col gap-1.5">
              ...
              >
                <span
                  class="inline-flex items-center gap-0.5 text-[10px] font-medium uppercase text-[color:var(--bx-text-dim)]"
