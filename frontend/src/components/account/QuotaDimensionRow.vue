...
<template>
  <div>
    ...
    <div v-if="quotaNotifyGlobalEnabled" class="flex items-center gap-2 mb-1">
      <span class="text-xs font-medium text-[color:var(--bx-text-soft)] flex-1 min-w-0">{{ label }}</span>
      <span v-if="limit && limit > 0" class="text-xs font-medium text-[color:var(--bx-text-soft)] flex-1 min-w-0">{{ t('admin.accounts.quotaNotify.alert') }}</span>
    </div>
    <label v-else class="text-xs font-medium text-[color:var(--bx-text-soft)] mb-1 block">{{ label }}</label>

    <!-- Input row -->
    <div class="flex items-center gap-2">
      <div :class="['relative', quotaNotifyGlobalEnabled ? 'flex-1 min-w-0' : 'flex-1']">
        <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-[color:var(--bx-text-dim)] text-sm">$</span>
    ...
    <div v-if="hasResetMode" class="mt-1 flex items-center gap-2 flex-wrap">
      <label class="text-xs text-[color:var(--bx-text-dim)] whitespace-nowrap">{{ t('admin.accounts.quotaResetMode') }}</label>
      ...
      <template v-if="resetMode === 'fixed'">
        ...
        <template v-if="dim === 'weekly'">
          <label class="text-xs text-[color:var(--bx-text-dim)] whitespace-nowrap">{{ t('admin.accounts.quotaWeeklyResetDay') }}</label>
        ...
        <label class="text-xs text-[color:var(--bx-text-dim)] whitespace-nowrap">{{ t('admin.accounts.quotaResetHour') }}</label>
      ...
      <span class="text-[11px] text-[color:var(--bx-text-dim)]">
