...
const tierBadgeClass = computed(() => {
  ...
  if (isGoogleOne.value) {
    ...
    if (tier === 'google_one_free') return 'bg-[color:var(--bx-bg-muted)] text-[color:var(--bx-text-muted)]  '
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
  ...
  if (tier === 'aistudio_free') return 'bg-[color:var(--bx-bg-muted)] text-[color:var(--bx-text-muted)]  '
  return 'bg-blue-100 text-blue-600 dark:bg-blue-900/40 dark:text-blue-300'
})
