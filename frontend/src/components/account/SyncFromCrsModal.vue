<template>
  ...
  >
    ...
    >
      ...
      <div
        class="rounded-lg bg-[color:var(--bx-bg-muted)] p-3 text-xs text-[color:var(--bx-text-dim)]  "
      >
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <label class="flex items-center gap-2 text-sm text-[color:var(--bx-text-soft)]">
          <input
            ...
            type="checkbox"
            class="rounded border-[color:var(--bx-border-strong)]"
          />
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      >
        <div class="mb-2 text-sm font-medium text-[color:var(--bx-text-soft)]">
          {{ t('admin.accounts.crsExistingAccounts') }}
          <span class="ml-1 text-xs text-gray-400">({{ previewResult.existing_accounts.length }})</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-[color:var(--bx-text-dim)]">
          <div
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          ...
          <div class="flex gap-2">
            ...
            <button
              type="button"
              class="text-xs text-gray-500 hover:text-[color:var(--bx-text-muted)]"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') }}</button>
          </div>
        </div>
        <div
          class="max-h-48 overflow-auto rounded-lg border border-[color:var(--bx-border)] p-2 "
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 hover:bg-[color:var(--bx-hover)]"
          >
            <input
              type="checkbox"
            ...
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-[color:var(--bx-text-soft)]">{{ acc.name }}</span>
          </label>
        </div>
      ...
      <div class="flex items-center gap-2 text-xs text-[color:var(--bx-text-dim)]">
        ...
        <span :class="form.sync_proxies ? 'text-green-600 dark:text-green-400' : 'text-[color:var(--bx-text-dim)]'">
      ...
      <div
        ...
        class="rounded-lg bg-[color:var(--bx-bg-muted)] p-4 text-center text-sm text-[color:var(--bx-text-dim)]  "
      >
