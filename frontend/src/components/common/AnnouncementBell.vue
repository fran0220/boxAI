<template>
  <div>
    ...
    <button
      @click="openModal"
      class="relative flex h-9 w-9 items-center justify-center rounded-lg text-[color:var(--bx-text-muted)] transition-all hover:bg-[color:var(--bx-hover)] hover:scale-105  "
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        ...
        >
          <div
            class="w-full max-w-[620px] overflow-hidden rounded-3xl bg-[color:var(--bx-bg-elevated)] shadow-2xl ring-1 ring-black/5 dark:ring-white/10"
            @click.stop
          >
            <!-- Header with Gradient -->
            <div class="relative overflow-hidden border-b border-[color:var(--bx-border)] bg-gradient-to-br from-blue-50/50 to-indigo-50/30 px-6 py-5 /50 dark:from-blue-900/10 dark:to-indigo-900/5">
              <div class="relative z-10 flex items-start justify-between">
                <div>
                  ...
                  <p v-if="unreadCount > 0" class="mt-2 text-sm text-[color:var(--bx-text-muted)]">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button
                    @click="closeModal"
                    class="flex h-9 w-9 items-center justify-center rounded-lg bg-white/50 text-[color:var(--bx-text-dim)] backdrop-blur-sm transition-all hover:bg-[color:var(--bx-bg-elevated)] hover:text-gray-700   dark:hover:bg-dark-700 dark:hover:text-gray-300"
            ...
            <div class="max-h-[65vh] overflow-y-auto">
              ...
              <div v-if="loading" class="flex items-center justify-center py-16">
                <div class="relative">
                  <div class="h-12 w-12 animate-spin rounded-full border-4 border-[color:var(--bx-border)] border-t-blue-600  dark:border-t-blue-400"></div>
              ...
              <div v-else-if="announcements.length > 0">
                <div
                  ...
                  :key="item.id"
                  class="group relative flex items-center gap-4 border-b border-[color:var(--bx-border)] px-6 py-4 transition-all hover:bg-[color:var(--bx-hover)]  "
                ...
                >
                  ...
                  <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center">
                    ...
                    <div
                      v-else
                      class="flex h-10 w-10 items-center justify-center rounded-xl bg-[color:var(--bx-bg-muted)] text-gray-400  dark:text-gray-600"
                    >
                  ...
                  <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
                    <div class="min-w-0 flex-1">
                      <h3 class="truncate text-sm font-medium text-[color:var(--bx-text)]">
                        {{ item.title }}
                      </h3>
                      <div class="mt-1 flex items-center gap-2">
                        <time class="text-xs text-[color:var(--bx-text-dim)]">
              ...
              <div v-else class="flex flex-col items-center justify-center py-16">
                <div class="relative mb-4">
                  <div class="flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-dark-700 dark:to-dark-600">
                    <Icon name="inbox" size="xl" class="text-[color:var(--bx-text-dim)]" />
                  </div>
                ...
                <p class="text-sm font-medium text-[color:var(--bx-text)]">{{ t('announcements.empty') }}</p>
                <p class="mt-1 text-xs text-[color:var(--bx-text-dim)]">{{ t('announcements.emptyDescription') }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        ...
        >
          <div
            class="w-full max-w-[780px] overflow-hidden rounded-3xl bg-[color:var(--bx-bg-elevated)] shadow-2xl ring-1 ring-black/5 dark:ring-white/10"
            @click.stop
          >
            <!-- Header with Decorative Elements -->
            <div class="relative overflow-hidden border-b border-[color:var(--bx-border)] bg-gradient-to-br from-blue-50/80 via-indigo-50/50 to-purple-50/30 px-8 py-6  dark:from-blue-900/20 dark:via-indigo-900/10 dark:to-purple-900/5">
              ...
              <div class="relative z-10 flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  ...
                  <!-- Title -->
                  <h2 class="mb-3 text-2xl font-bold leading-tight text-[color:var(--bx-text)]">
                    {{ selectedAnnouncement.title }}
                  </h2>

                  <!-- Meta Info -->
                  <div class="flex items-center gap-4 text-sm text-[color:var(--bx-text-muted)]">
                ...
                <button
                  @click="closeDetail"
                  class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-white/50 text-[color:var(--bx-text-dim)] backdrop-blur-sm transition-all hover:bg-[color:var(--bx-bg-elevated)] hover:text-gray-700 hover:shadow-lg   dark:hover:bg-dark-700 dark:hover:text-gray-300"
                  :aria-label="t('common.close')"
                >
                  <Icon name="x" size="md" />
                </button>
            ...
            <div class="max-h-[60vh] overflow-y-auto bg-[color:var(--bx-bg-elevated)] px-8 py-8 ">
            ...
            <div class="border-t border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)] px-8 py-5  ">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-xs text-[color:var(--bx-text-dim)]">
                  <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                ...
                <div class="flex items-center gap-3">
                  <button
                    @click="closeDetail"
                    class="rounded-xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] px-5 py-2.5 text-sm font-medium text-[color:var(--bx-text-soft)] shadow-sm transition-all hover:bg-[color:var(--bx-hover)] hover:shadow    "
                  >
                    {{ t('common.close') }}
                  </button>
                  <button
...
.markdown-body h1 {
  @apply mb-6 mt-8 border-b border-[color:var(--bx-border)] pb-3 text-3xl font-bold text-[color:var(--bx-text)]  ;
}

.markdown-body h2 {
  @apply mb-4 mt-7 border-b border-[color:var(--bx-border)] pb-2 text-2xl font-bold text-[color:var(--bx-text)]  ;
}
...
.markdown-body h4 {
  @apply mb-2 mt-5 text-lg font-semibold text-[color:var(--bx-text)];
}
...
.markdown-body blockquote {
  @apply relative my-5 border-l-4 border-blue-500 bg-blue-50/50 py-3 pl-5 pr-4 italic text-[color:var(--bx-text-soft)] dark:border-blue-400 dark:bg-blue-900/10 ;
}
...
.markdown-body code {
  @apply rounded-lg bg-[color:var(--bx-bg-muted)] px-2 py-1 text-[13px] font-mono text-pink-600  dark:text-pink-400;
}

.markdown-body pre {
  @apply my-5 overflow-x-auto rounded-xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)] p-5  ;
}
...
.markdown-body hr {
  @apply my-8 border-0 border-t-2 border-[color:var(--bx-border)];
}

.markdown-body table {
  @apply mb-5 w-full overflow-hidden rounded-lg border border-[color:var(--bx-border)];
}

.markdown-body th,
.markdown-body td {
  @apply border-r border-b border-[color:var(--bx-border)] px-4 py-3 text-left ;
}
...
.markdown-body th {
  @apply bg-gradient-to-br from-blue-50 to-indigo-50 font-semibold text-[color:var(--bx-text)] dark:from-blue-900/20 dark:to-indigo-900/10 ;
}

.markdown-body tbody tr {
  @apply transition-colors hover:bg-[color:var(--bx-hover)];
}

.markdown-body img {
  @apply my-5 max-w-full rounded-xl border border-[color:var(--bx-border)] shadow-md ;
}

.markdown-body strong {
  @apply font-semibold text-[color:var(--bx-text)];
}

.markdown-body em {
  @apply italic text-[color:var(--bx-text-muted)];
}
</style>
