<template>
  <div v-if="!isDesktopViewport" class="space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-4  ">
        <div class="space-y-3">
          <div v-for="column in dataColumns" :key="column.key" class="flex justify-between">
            <div class="h-4 w-20 animate-pulse rounded bg-[color:var(--bx-bg-muted)]"></div>
            <div class="h-4 w-32 animate-pulse rounded bg-[color:var(--bx-bg-muted)]"></div>
          </div>
          <div v-if="hasActionsColumn" class="border-t border-[color:var(--bx-border)] pt-3 ">
            <div class="h-8 w-full animate-pulse rounded bg-[color:var(--bx-bg-muted)]"></div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-12 text-center  ">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-[color:var(--bx-text-dim)]"
            />
            <p class="text-lg font-medium text-[color:var(--bx-text)]">
              {{ t('empty.noData') }}
            </p>
          </div>
        </slot>
    ...
    <template v-else>
      ...
      >
        <div class="space-y-3">
          ...
          >
            <span class="text-xs font-medium uppercase tracking-wider text-[color:var(--bx-text-dim)]">
              {{ column.label }}
            </span>
            <div class="text-right text-sm text-[color:var(--bx-text)]">
  ...
  >
    <table class="w-full min-w-max divide-y divide-[color:var(--bx-border)]">
      <thead class="table-header bg-[color:var(--bx-bg-muted)]">
        <tr>
          <th
            ...
            :class="[
              'sticky-header-cell py-3 text-left text-xs font-medium uppercase tracking-wider text-[color:var(--bx-text-dim)]',
              getAdaptivePaddingClass(),
              { 'cursor-pointer hover:bg-[color:var(--bx-hover)]': column.sortable },
      ...
      <tbody class="table-body divide-y divide-gray-200 bg-[color:var(--bx-bg-elevated)] dark:divide-dark-700 ">
        <!-- Loading skeleton -->
        <tr v-if="loading" v-for="i in 5" :key="i">
          <td v-for="column in columns" :key="column.key" :class="['whitespace-nowrap py-4', getAdaptivePaddingClass()]">
            <div class="animate-pulse">
              <div class="h-4 w-3/4 rounded bg-[color:var(--bx-bg-muted)]"></div>
            </div>
          </td>
        </tr>
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-[color:var(--bx-text-dim)]', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          >
            <td
              ...
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-[color:var(--bx-text)]',
                getAdaptivePaddingClass(),
