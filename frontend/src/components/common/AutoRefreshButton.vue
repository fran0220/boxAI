<template>
  <div class="relative" ref="dropdownRef">
    <button
      ...
      class="inline-flex items-center gap-1.5 rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] px-2.5 py-1.5 text-xs font-medium text-[color:var(--bx-text-soft)] shadow-sm transition-colors hover:bg-[color:var(--bx-hover)]    "
    ...
    <div
      v-if="showDropdown"
      class="absolute right-0 z-20 mt-1 w-44 rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] shadow-lg"
    >
      <div class="p-1.5">
        <button
          @click="$emit('update:enabled', !enabled)"
          class="flex w-full items-center justify-between rounded-md px-3 py-1.5 text-sm text-[color:var(--bx-text-soft)] hover:bg-gray-100  dark:hover:bg-gray-700"
        >
