<template>
  ...
  <div>
    <!-- Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-[color:var(--bx-border)] px-4 py-3 /50 sm:px-6">
      <p class="text-xs text-[color:var(--bx-text-dim)]">{{ t('admin.usage.tokenRanking.subtitle') }}</p>
      <div class="flex items-center gap-3">
        <span v-if="!loading && items.length > 0" class="text-xs text-[color:var(--bx-text-dim)]">
    ...
    <div class="overflow-x-auto">
      <table class="w-full min-w-max divide-y divide-[color:var(--bx-border)]">
        <thead class="bg-[color:var(--bx-bg-muted)]">
          <tr>
            <th class="w-16 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-[color:var(--bx-text-dim)] sm:px-6">#</th>
            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-[color:var(--bx-text-dim)]">
              {{ t('admin.usage.tokenRanking.columns.user') }}
            </th>
            <th
              v-for="col in sortableColumns"
              :key="col.key"
              class="cursor-pointer select-none whitespace-nowrap px-4 py-3 text-right text-xs font-medium uppercase tracking-wider transition-colors hover:bg-[color:var(--bx-hover)]"
              :class="sortBy === col.key ? 'text-primary-400' : 'text-[color:var(--bx-text-dim)]'"
        ...
        <tbody class="divide-y divide-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)]">
          ...
          <tr
            ...
            v-else
            :key="item.user_id"
            class="cursor-pointer transition-colors hover:bg-[color:var(--bx-hover)]"
          ...
          >
            ...
            <td class="max-w-[260px] truncate px-4 py-3 text-sm font-medium text-[color:var(--bx-text-soft)]" :title="item.email">
              {{ item.email || `User #${item.user_id}` }}
              <span class="ml-1 font-normal text-[color:var(--bx-text-dim)]">#{{ item.user_id }}</span>
            </td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-[color:var(--bx-text-dim)]">{{ item.requests.toLocaleString() }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-[color:var(--bx-text-dim)]">{{ fmtTokens(item.input_tokens) }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-[color:var(--bx-text-dim)]">{{ fmtTokens(item.output_tokens) }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-[color:var(--bx-text-dim)]">{{ fmtTokens(item.cache_tokens) }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm font-medium tabular-nums text-[color:var(--bx-text)]">{{ fmtTokens(item.total_tokens) }}</td>
