    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)] p-3  ">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-[color:var(--bx-text)]">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-gray-400" />
          <span v-if="selectedGroupId" class="font-medium text-primary-400">
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        <label
          ...
          :class="selectedGroupId === group.id
            ...
            : 'border-gray-200 hover:border-[color:var(--bx-border-strong)] dark:hover:border-dark-500'"
        >
          <input
            type="radio"
            :value="group.id"
