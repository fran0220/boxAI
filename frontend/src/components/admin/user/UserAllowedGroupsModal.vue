  ...
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      ...
      <div class="flex items-center gap-4 rounded-2xl bg-gradient-to-r from-primary-50 to-primary-100 p-5 dark:from-primary-900/30 dark:to-primary-800/20">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-[color:var(--bx-bg-elevated)] shadow-sm">
          <span class="text-2xl font-semibold text-primary-400">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-[color:var(--bx-text)]">{{ user.email }}</p>
          <p class="mt-1 text-sm text-[color:var(--bx-text-muted)]">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-[color:var(--bx-text-soft)]">{{ t('admin.users.exclusiveGroups') }}</h4>
          ...
          <div class="grid gap-3">
            <div
              ...
              :class="config.isSelected
                ...
                : 'border-gray-200 bg-[color:var(--bx-bg-elevated)] hover:border-[color:var(--bx-border-strong)]  dark:hover:border-dark-500'"
            >
              <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center">
                    <input
                      ...
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-md border-2 border-[color:var(--bx-border)] transition-all peer-checked:border-primary-500 peer-checked:bg-primary-500  peer-checked:dark:border-primary-500">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-[color:var(--bx-text)]">{{ config.groupName }}</span>
                  ...
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    <span class="inline-flex items-center gap-1 text-[color:var(--bx-text-dim)]">
                      <PlatformIcon :platform="config.platform" size="xs" />
                      <span>{{ config.platform }}</span>
                    </span>
                    <span class="text-gray-300 dark:text-dark-500">•</span>
                    <span class="text-[color:var(--bx-text-dim)]">
                      {{ t('admin.users.defaultRate') }}: <span class="font-medium text-[color:var(--bx-text-soft)]">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-[color:var(--bx-text-muted)]">{{ t('admin.users.customRate') }}</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner w-24 rounded-lg border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] px-3 py-2 text-sm font-medium transition-colors focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20   dark:focus:border-primary-500"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-[color:var(--bx-text-soft)]">{{ t('admin.users.publicGroups') }}</h4>
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[color:var(--bx-bg-muted)]">
            <svg class="h-8 w-8 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <p class="text-[color:var(--bx-text-dim)]">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
