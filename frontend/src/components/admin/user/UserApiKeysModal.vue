<template>
  <BaseDialog :show="show" :title="t('admin.users.userApiKeys')" width="wide" @close="handleClose">
    <div v-if="user" class="space-y-4">
      <div class="flex items-center gap-3 rounded-xl bg-[color:var(--bx-bg-muted)] p-4 ">
        <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-900/30">
          <span class="text-lg font-medium text-primary-300">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div><p class="font-medium text-[color:var(--bx-text)]">{{ user.email }}</p><p class="text-sm text-[color:var(--bx-text-dim)]">{{ user.username }}</p></div>
      </div>
      ...
      <div v-else ref="scrollContainerRef" class="max-h-96 space-y-3 overflow-y-auto" @scroll="closeGroupSelector">
        <div v-for="key in apiKeys" :key="key.id" class="rounded-xl border border-[color:var(--bx-border)] bg-[color:var(--bx-bg-elevated)] p-4  ">
          <div class="flex items-start justify-between">
            <div class="min-w-0 flex-1">
              <div class="mb-1 flex items-center gap-2"><span class="font-medium text-[color:var(--bx-text)]">{{ key.name }}</span><span :class="['badge text-xs', key.status === 'active' ? 'badge-success' : 'badge-danger']">{{ key.status }}</span></div>
          ...
          <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-500">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="-mx-1 -my-0.5 flex cursor-pointer items-center gap-1 rounded-md px-1 py-0.5 transition-colors hover:bg-[color:var(--bx-hover)]"
  ...
  <Teleport to="body">
    <div
      ...
      ref="dropdownRef"
      class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-64 overflow-hidden rounded-xl bg-[color:var(--bx-bg-elevated)] shadow-lg ring-1 ring-black/5 duration-200  dark:ring-white/10"
