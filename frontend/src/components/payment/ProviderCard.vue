<template>
  <div
    :class="[
      ...
      enabled ? 'border-[color:var(--bx-border)]' : 'border-[color:var(--bx-border)] bg-[color:var(--bx-bg-muted)] opacity-50  ',
    ]"
  ...
  >
    <div :class="[
      ...
      <div class="flex items-center gap-3">
        <div :class="[
          'rounded-md p-1.5',
          provider.enabled && enabled ? 'bg-green-100 dark:bg-green-900/30' : 'bg-[color:var(--bx-bg-muted)]',
        ]">
          <Icon
            name="server"
            size="sm"
        ...
        <span class="text-sm font-medium text-[color:var(--bx-text)]">{{ provider.name }}</span>
        <span class="text-xs text-[color:var(--bx-text-dim)]">{{ keyLabel }}</span>
        <span v-if="provider.payment_mode" class="text-xs text-[color:var(--bx-text-dim)]">· {{ modeLabel }}</span>
          ...
          <button
            ...
            :class="[
              ...
              isSelected(pt.value)
                ...
                : 'bg-[color:var(--bx-bg-muted)] text-gray-400  dark:text-gray-500',
            ]"
          >{{ pt.label }}</button>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2 border-l border-[color:var(--bx-border)] pl-3 ">
