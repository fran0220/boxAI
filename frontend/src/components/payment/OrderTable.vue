<template>
  <DataTable :columns="columns" :data="orders" :loading="loading">
    ...
    <template #cell-out_trade_no="{ value }">
      <span class="text-sm text-[color:var(--bx-text)]">{{ value }}</span>
    </template>
    <template v-if="showUser" #cell-user_email="{ value, row }">
      <div class="text-sm">
        <span class="text-[color:var(--bx-text)]">{{ value || row.user_name || '#' + row.user_id }}</span>
    ...
    <template #cell-payment_type="{ value }">
      <span class="text-sm text-[color:var(--bx-text-soft)]">{{ t('payment.methods.' + value, value) }}</span>
    </template>
    <template #cell-status="{ value }">
      <OrderStatusBadge :status="value" />
    </template>
    <template #cell-created_at="{ value }">
      <span class="text-xs text-[color:var(--bx-text-dim)]">{{ formatDate(value) }}</span>
    </template>
