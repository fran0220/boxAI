...
export const translations: Record<Locale, Record<string, string>> = {
  "zh-CN": {
    ...
    "settings.systemToolsBuiltinDesc":
      "BoxAI 在 Agent 模式下自动注册的内置工具，此处仅供查阅。",
    ...
    "settings.remoteDesc": "通过云端 Gateway 远程控制本地 BoxAI",
    ...
    "settings.remoteInfoBanner":
      "启用后，本地 BoxAI 将通过 gRPC 双向流连接云端 Gateway。你可以在浏览器中通过 WebUI 远程发送 Chat 消息、管理 Cron 任务和查看历史记录。所有工具执行仍在本地完成，远程端仅转发指令和结果。",
    ...
    "settings.skillsImportDesc":
      "扫描本机 Claude Code、Codex、CodeBuddy 的技能目录，勾选后导入到 BoxAI。",
  ...
  "en-US": {
    ...
    "settings.systemToolsBuiltinDesc":
      "Built-in tools that BoxAI registers automatically in Agent mode. Read-only list; click the eye icon for details.",
    "settings.systemToolsCustomDesc":
    ...
    "settings.remoteDesc": "Control your local BoxAI remotely via cloud Gateway",
    ...
    "settings.remoteInfoBanner":
      "When enabled, the local BoxAI connects to the cloud Gateway via gRPC bidirectional streaming. You can remotely send Chat messages, manage Cron tasks, and view history through the WebUI in your browser. All tool execution remains local — the remote end only relays commands and results.",
    ...
    "settings.skillsImportDesc":
      "Scan local Claude Code, Codex, and CodeBuddy skill directories, then pick skills to import into BoxAI.",
