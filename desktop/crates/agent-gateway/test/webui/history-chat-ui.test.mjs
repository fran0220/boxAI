...
test("WebUI transcript strips leaked DSML tool call markup from text and thinking", () => {
  const dsml = [
    ...
    '<||DSML|| parameter name="query">BoxAI DSML markup</||DSML|| parameter>',
    "</||DSML|| invoke>",
    "</||DSML|| tool_calls>",
  ].join("\n");
  const entries = chatUi.parseHistoryMessagesJson(JSON.stringify([
...
test("WebUI transcript hides provider-native web_search tool traces when hosted search exists", () => {
  const webSearchCall = {
    ...
    name: "web_search",
    arguments: { query: "BoxAI DeepSeek webui search" },
  };
  const entries = chatUi.parseHistoryMessagesJson(JSON.stringify([
    { role: "user", content: "search" },
    {
      ...
      content: [
        { type: "text", text: "searching" },
        {
          ...
          status: "completed",
          queries: ["BoxAI DeepSeek webui search"],
...
test("WebUI live transcript removes provider-native web_search when hosted search arrives later", () => {
  ...
  turn = applyEventToTurn(turn, {
    ...
    name: "web_search",
    arguments: { query: "BoxAI DeepSeek live search" },
    round: 1,
  });
  ...
  turn = applyEventToTurn(turn, {
    ...
    provider: "claude_code",
    status: "completed",
    queries: ["BoxAI DeepSeek live search"],
  ...
  const turn = reduceTurnEvents([
    {
      ...
      name: "WebSearch",
      arguments: { query: "BoxAI recovered search" },
      round: 1,
    },
    {
      type: "tool_result",
  ...
  const turn = reduceTurnEvents([
    {
      ...
      arguments: { query: "BoxAI DSML hidden search" },
      round: 1,
    },
  ]);
