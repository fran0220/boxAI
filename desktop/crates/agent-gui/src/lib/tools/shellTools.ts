...
function validateBashBackgroundStdio(command: string) {
  ...
  throw new Error(
    [
      ...
      "Long-running processes that inherit BoxAI's tool pipes can keep the Bash task running forever.",
...
export function createShellTools(params: {
  ...
  const toolManagedProcess: Tool = {
    name: "ManagedProcess",
    description: `Start, inspect, read logs for, or stop a long-running local process such as a dev server, watcher, or preview server. Runtime platform: ${platformLabel}; commands use the same platform shell policy as Bash. Use this instead of detached shell/background syntax. action="start" runs a foreground command under BoxAI process management, redirects stdout/stderr to a log file, and returns immediately with process_id, pid, and log_path. By default managed processes are terminated automatically when BoxAI exits; pass isolated=true only when the user explicitly wants the service to outlive BoxAI. Use action="status" to list or inspect processes, action="read_log" to read recent log output, and action="stop" to terminate the process tree.`,
    parameters: strictToolParameters({
      action: Type.Union(
        [
          Type.Literal("start"),
      ...
      isolated: Type.Optional(
        Type.Boolean({
          description:
            'Only for action="start". Default false: the process is terminated automatically when BoxAI exits. Set true ONLY when the user explicitly asks for the service to keep running after BoxAI quits; it then detaches from the BoxAI lifecycle and must be stopped manually from the background tasks panel.',
        }),
      ),
