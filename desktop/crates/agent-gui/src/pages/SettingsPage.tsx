...
export function SettingsPage(props: SettingsPageProps) {
  ...
  const accountServerLabel = useMemo(() => {
    if (!account) return "";
    try {
      return new URL(account.serverUrl).host;
    } catch {
            ...
            <div className="border-t border-border/60 px-3 py-3">
              <div className="px-1">
                <div className="truncate text-xs font-medium text-foreground">
                  {account.user.email || account.user.username || "BoxAI account"}
                </div>
                <div
              ...
              <button
                ...
                className="mt-2 w-full rounded-lg border border-border px-2.5 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-accent/60"
              >
                Sign out
              </button>
            </div>
