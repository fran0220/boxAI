...
Use this reference before installing or replacing BoxAI skills.
...
- Parse `name` and `description` before installing. For `README.md` without frontmatter, BoxAI derives the skill name from the directory and loads the full README when enabled.
...
- Do not manually edit BoxAI Settings. SkillsManager automatically enables newly installed skills for the current conversation when installation succeeds.

## After Install

BoxAI discovers skills through a scan of the fixed skills root. After an install, BoxAI refreshes discovery; if the UI still has stale state:
