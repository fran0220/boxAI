        ...
        .on_menu_event(move |app, event| match event.id().as_ref() {
            TRAY_SHOW_ID => {
                if let Err(error) = show_main_window(app) {
                    eprintln!("failed to show BoxAI window from tray: {error}");
                }
            }
            TRAY_QUIT_ID => {
                ...
                if running_count > 0 {
                    if let Err(error) = show_main_window(app) {
                        eprintln!(
                            "failed to show BoxAI window before terminal exit confirm: {error}"
                        );
                    }
            ...
            move |tray, event| match event {
                TrayIconEvent::DoubleClick {
                    ...
                    if let Err(error) = show_main_window(tray.app_handle()) {
                        eprintln!("failed to show BoxAI window from tray double-click: {error}");
                    }
                }
                TrayIconEvent::Click {
                    button: MouseButton::Left,
                    ...
                    if record_tray_left_click(&last_left_click_at) {
                        if let Err(error) = show_main_window(tray.app_handle()) {
                            eprintln!(
                                "failed to show BoxAI window from tray left double-click: {error}"
                            );
                        }
                    }
                }
...
pub fn run() {
    let automation_store = Arc::new(
        services::automation::AutomationStore::open()
            .expect("failed to initialize BoxAI automation store"),
    );
    let automation_scheduler = Arc::new(services::automation::AutomationScheduler::new(
        Arc::clone(&automation_store),
    ));
    let memory_store = Arc::new(
        services::memory::MemoryStore::open().expect("failed to initialize BoxAI memory store"),
    );
        ...
        .on_window_event(|window, event| {
            ...
            if let WindowEvent::CloseRequested { api, .. } = event {
                ...
                if let Err(error) = window.hide() {
                    eprintln!("failed to hide BoxAI window on close: {error}");
                }
            }
        })
    ...
    app.run(move |_app, event| match event {
        ...
        tauri::RunEvent::Reopen { .. } => {
            if let Err(error) = show_main_window(_app) {
                eprintln!("failed to show BoxAI window from dock reopen: {error}");
            }
        }
