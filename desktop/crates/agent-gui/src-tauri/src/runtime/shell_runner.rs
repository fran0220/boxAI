...
pub(crate) fn run_shell_script_with_envs(
    ...
    if stdio_open_after_exit {
        ...
        if shell_profile.platform == "windows" {
            stderr_str.push_str(
                "BoxAI warning: command exited, but stdout/stderr remained open after exit. \
This usually means a background process inherited the tool pipes. Use ManagedProcess for \
long-running Windows commands so BoxAI can capture logs and stop the process tree.",
            );
        } else {
            stderr_str.push_str(
                "BoxAI warning: command exited, but stdout/stderr remained open after exit. \
