...
test("hosted search aggregation dedupes identical updates and separates completion from dispose", () => {
  ...
  const rawSearchEvent = {
    ...
    item: {
      ...
      id: "search-1",
      status: "in_progress",
      action: { query: "BoxAI hosted search" },
    },
  };
...
test("hosted search aggregation extracts structural Anthropic and Gemini search metadata", () => {
  ...
  anthropic.accept({
    ...
    content_block: {
      ...
      id: "toolu_1",
      name: "web_search",
      input: { query: "BoxAI Anthropic search" },
    },
  });
  anthropic.accept({
    type: "content_block_start",
  ...
  assert.deepEqual(anthropic.getBlocks()[0].queries, ["BoxAI Anthropic search"]);
  ...
  gemini.accept({
    candidates: [
      {
        groundingMetadata: {
          webSearchQueries: ["BoxAI Gemini search"],
          groundingChunks: [
            {
              web: {
  ...
  assert.deepEqual(gemini.getBlocks()[0].queries, ["BoxAI Gemini search"]);
...
test("hosted search aggregation accumulates an Anthropic query streamed as partial_json fragments", () => {
  ...
  assert.deepEqual(anthropic.getBlocks()[0].queries, ["BoxAI incremental search"]);
...
test("hosted search aggregation extracts Anthropic citations_delta into the active search block", () => {
  ...
  anthropic.accept({
    ...
    content_block: {
      ...
      id: "toolu_cited",
      name: "web_search",
      input: { query: "BoxAI citation search" },
    },
  });
  anthropic.accept({
    type: "content_block_delta",
  ...
  assert.deepEqual(block.queries, ["BoxAI citation search"]);
  assert.deepEqual(
...
test("hosted search aggregation marks Anthropic web_search_tool_result_error as failed", () => {
  ...
  anthropic.accept({
    ...
    content_block: {
      ...
      id: "toolu_failed",
      name: "web_search",
      input: { query: "BoxAI failing search" },
    },
  });
  anthropic.accept({
    type: "content_block_start",
...
test("hosted search aggregation extracts OpenAI url_citation annotations and marks call completion", () => {
  ...
  openai.accept({
    ...
    item: {
      ...
      id: "search-annotated",
      status: "in_progress",
      action: { query: "BoxAI OpenAI search" },
    },
  });
  openai.accept({
  ...
  assert.deepEqual(block.queries, ["BoxAI OpenAI search"]);
  assert.deepEqual(
    block.sources.map((source) => ({ url: source.url, sourceType: source.sourceType })),
